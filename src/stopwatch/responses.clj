...
(defn already-watch
  [elapsed-time]
  (response/respond (response/say "Your stopwatch is already running.")
                    (response/card
...
(defn no-watch
  ...
  ([started?]
   (let [speech-text (if started? "No stopwatch is set, but I started a new one." "No stopwatch is set.")
     ...
     (response/respond (response/say speech-text)
                       (response/card "No Stopwatch Running" card-text)))))

(defn restarted-watch
  [elapsed-time] (response/respond (response/say (str "Stopwatch restarted. Previous duration was " (verbal-status elapsed-time)))
                                   (response/card "Stopwatch Restarted"
                                                  (str "Duration: " (format-duration elapsed-time)))))
(defn paused-watch
  [elapsed-time]
  (response/respond (response/say (str "Stopwatch paused. Current duration is " (verbal-status elapsed-time)))
                    (response/card "Stopwatch Paused"
                                   (str "Duration: " (format-duration elapsed-time)))))

(defn already-paused-watch [elapsed-time]
  (response/respond
    (response/say (str "Stopwatch already paused at " (verbal-status elapsed-time)))
    (response/card "Stopwatch Already Paused"
                   (str "Duration: " (format-duration elapsed-time)))))

(defn resumed-watch [elapsed-time]
  (response/respond
    (response/say "Resuming your stopwatch.")
    (response/card "Stopwatch Resumed"
                   (str "Duration: " (format-duration elapsed-time)))))